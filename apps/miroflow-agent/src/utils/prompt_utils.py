...
1. When the question involves images of specific entities, prefer using the visual_search tool to search for relevant information instead of relying solely on your internal knowledge.

2. If an image contains a large amount of information but only a specific region is relevant, first use the zoom_in tool to focus on that region. After verifying the correct area, apply visual_search on the cropped region. This often yields better results than searching the entire image.

3. If the search results or webpage content contain image URLs that are likely helpful for solving the task, prioritize using the fetch_image tool to obtain the original image for further analysis.
